...
function FSGridBookmarklet() {
		...
		Overlay = function () {
			...
			if ($jq('.fs-grid').length < 1) {
				...
				if ($jq('#fs-grid-styles').length < 1) {
					$jq('body').append(
						'<link id="fs-grid-styles" rel="stylesheet" href="//formstone.it/bookmarklet/grid.css" type="text/css" media="all">'
					);
				}
				...
				if ($jq('#fs-grid-overlay').length < 1) {
					...
					html +=
						'<aside id="fs-grid-menu" class="' +
						config.position +
						'">';

					html += '<div class="fs-grid-statuses">';
					html +=
						'<span class="fs-grid-status fs-grid-status-xs">xs</span>';
					html +=
						'<span class="fs-grid-status fs-grid-status-sm">sm</span>';
					html +=
						'<span class="fs-grid-status fs-grid-status-md">md</span>';
					html +=
						'<span class="fs-grid-status fs-grid-status-lg">lg</span>';
					html +=
						'<span class="fs-grid-status fs-grid-status-xl">xl</span>';
					html +=
						'<span class="fs-grid-status fs-grid-status-xxl">xxl</span>';
					html += '</div>';

					html +=
						'<button class="fs-grid-show fs-grid-option">Show Grid</button>';
					html +=
						'<button class="fs-grid-icon fs-grid-remove">Close</button>';
					html += '</aside>';
					html +=
						'<section id="fs-grid-overlay" aria-label="Grid Overlay">';
					html += '<div class="fs-row">';
					for (var i = 0; i < desktopCount; i++) {
						html +=
							'<div class="fs-cell fs-sm-1 fs-md-1 fs-lg-1"><span></span></div>';
					}
					html += '</div>';
					html += '</section>';
					...
					_this.$menu
						.on(
							'click',
							'.fs-grid-option',
							$jq.proxy(_this.onClick, _this)
						)
						.on(
							'click',
							'.fs-grid-remove',
							$jq.proxy(_this.remove, _this)
						);

					if (
						config.onLoad ||
						(config.useCookies === true &&
							_this.readCookie('fs-grid-active') === 'true')
					) {
						_this.$menuItems
							.filter('.fs-grid-show')
							.trigger('click');
	...
	Overlay.prototype = {
		onClick: function (e) {
			...
			if (
				$target.hasClass('fs-grid-active') ||
				$target.hasClass('fs-grid-remove')
			) {
				$target.removeClass('fs-grid-active').html('Show Grid');
				_this.$overlay.removeClass('fs-grid-visible');
				_this.eraseCookie('fs-grid-active');
			} else {
				$target.addClass('fs-grid-active').html('Hide Grid');
...
window.FSGridBookmarklet = FSGridBookmarklet;
